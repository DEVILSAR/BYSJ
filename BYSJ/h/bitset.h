 ...
 
 {
	...
	public:
		...
		reference& operator=(const reference& _Bs)
			{_Pbs->set(_Off, bool(_Bs));// pbs：bitset指针
			return (*this); }
		reference& flip()
			{_Pbs->flip(_Off);
			return (*this); }
...
//判断是否存在
	...
	_Ty _W(size_t _I) const
		{return (_A[_I]); }
private:
	enum {_Nb = CHAR_BIT * sizeof (_Ty), //char_BIT = 8  Nb = 32
		_Nw = _N == 0 ? 0 : (_N - 1) / _Nb}; 
						//Nb = bit number Nw=word number
	void _Tidy(_Ty _X = 0)//清除
		{for (int _I = _Nw; 0 <= _I; --_I)
			_A[_I] = _X;
		if (_X != 0)
			_Trim(); }
